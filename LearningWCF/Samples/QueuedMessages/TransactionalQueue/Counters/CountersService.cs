...
namespace Counters
{
    ...
    public class CountersService : ICountersService, ICountersAdmin
    {
        ...
        {
            Console.WriteLine("About to reset counters to {0} on transaction id {1}", 0, Transaction.Current.TransactionInformation.DistributedIdentifier);
            ...
            Console.WriteLine("Reset counters to {0} on transaction id {1}", 0, Transaction.Current.TransactionInformation.DistributedIdentifier);
        }

        [OperationBehavior(TransactionScopeRequired = true)]
        public void SetCounter1(int counterValue)
        {
            Console.WriteLine("About to reset counter 1 to {0} on transaction id {1}", counterValue, Transaction.Current.TransactionInformation.DistributedIdentifier);

            using (Counters.Dalc.CountersDataAccess countersDataAccess = new Counters.Dalc.CountersDataAccess())
            {
                countersDataAccess.SetCounter1(counterValue);
            }
            Console.WriteLine("Reset counter 1 to {0} on transaction id {1}", counterValue, Transaction.Current.TransactionInformation.DistributedIdentifier);
        }
